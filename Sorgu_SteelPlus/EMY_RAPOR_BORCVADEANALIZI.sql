...
--EXEC  EMY_RAPOR_BORCVADEANALIZI '','003','01','20200101','20201001','%%' ,5000


ALTER PROCEDURE [dbo].[EMY_RAPOR_BORCVADEANALIZI] @DB NVARCHAR(50), @FIRM NVARCHAR(3), @DONEM NVARCHAR(2) ,
...
BEGIN
     ...
     SET @PARAMETRELER = N'@_BASTAR DATETIME,@_BITTAR DATETIME,@_CARIKOD NVARCHAR(50),@_BAKIYE FLOAT';
     ...
     SET @QUERY1 = 'DECLARE @TARIH_1 DATE
...
SET @TABLO_SAY = (SELECT COUNT(id) FROM ..sysobjects WHERE  NAME LIKE ''EMY_PAYTRANS'')
IF @TABLO_SAY > 0 
	BEGIN
		DROP TABLE ..EMY_PAYTRANS
	END
	SELECT * INTO  ..EMY_PAYTRANS 
	FROM (
...
CLFLINE.SOURCEFREF FICHEREF,CLFLINE.SIGN,0 PAIDINCASH,0 CLOSINGRATE,CLFLINE.CANCELLED
 FROM {DATABASE}LG_{FIRMA}_{DONEM}_CLFLINE	CLFLINE WHERE CLFLINE.MODULENR=5 AND CLFLINE.TRCODE=6 AND CLFLINE.CANCELLED=0
		  ...
		  CREATE UNIQUE INDEX [LOGICALREF] ON ..EMY_PAYTRANS (LOGICALREF ASC)
		  CREATE INDEX CARDREF ON ..EMY_PAYTRANS (CARDREF ASC)
...
DECLARE Borc Cursor For
	...
	FROM   ..EMY_PAYTRANS WHERE CARDREF = @CLIENTREF AND SIGN =0 AND CANCELLED=0 AND PAIDINCASH = 0 AND CLOSINGRATE = 0 AND PROCDATE BETWEEN @TARIH_1 AND @TARIH_2   
	ORDER BY CARDREF,DATE_,FTIME
	
DECLARE Alacak Cursor For
	SELECT LOGICALREF,CARDREF,DATE_,PROCDATE,TRCODE,TOTAL  
	FROM   ..EMY_PAYTRANS WHERE CARDREF = @CLIENTREF AND SIGN =1 AND CANCELLED=0 AND PAIDINCASH = 0 AND CLOSINGRATE = 0 AND PROCDATE BETWEEN @TARIH_1 AND @TARIH_2   
	ORDER BY CARDREF,DATE_,FTIME
OPEN Alacak
	...
	OPEN Borc
		FETCH NEXT FROM Borc INTO @B_LOGICALREF,@B_CARDREF,@B_DATE,@B_PROCDATE,@B_TRCODE,@B_TOTAL
			...
			WHILE @@FETCH_STATUS =0
				BEGIN   
TEKRAR:'
	SET @QUERY4 = '				SET @B_KALAN = @B_KALAN - @A_KALAN 
					IF @B_KALAN > 0 
						BEGIN
							IF @A_KALAN > 0 
								BEGIN
							...
							FETCH NEXT FROM ALACAK INTO @A_LOGICALREF,@A_CARDREF,@A_DATE,@A_PROCDATE,@A_TRCODE,@A_TOTAL
								IF @@FETCH_STATUS = -1  -- Alacak Kaydı bulunamazsa iþlemi boþ geçiyor
									BEGIN 
										...
										INSERT INTO ..EMY_BorcTakip (CLIENTREF,B_PAYPLANREF,B_VADE_TARIHI,B_GIRIS_TARIHI,B_TUTAR,K_PAYPLANREF,K_VADE_TARIHI,K_GIRIS_TARIHI,K_ISLEM_TUTAR)
										SELECT CARDREF,	LOGICALREF,DATE_,PROCDATE ,TOTAL,null ,NULL ,null ,0  FROM ..EMY_PAYTRANS WHERE CARDREF = @CLIENTREF AND SIGN = 0 AND CANCELLED =0 AND PAIDINCASH = 0 AND  CLOSINGRATE = 0 AND  PROCDATE BETWEEN @TARIH_1 AND @TARIH_2 AND  LOGICALREF NOT IN (SELECT B_PAYPLANREF  FROM ..EMY_BorcTakip WHERE CLIENTREF =  @CLIENTREF)
...
-- 2
	CLCARD.DEFINITION_ [CariHesapUnvani],
CLCARD.SPECODE   [OzelKod],
...
-- 10	
	CONVERT(DATE,(SELECT TOP 1 DATE_ FROM ..{DATABASE}LG_{FIRMA}_{DONEM}_CLFLINE WHERE CLIENTREF = CLCARD.LOGICALREF AND DATE_ BETWEEN @TARIH_1 AND @TARIH_2 AND SIGN = 1 AND CANCELLED =0 ORDER BY DATE_ DESC,LOGICALREF DESC )) [SonTahsilatTarihi],
--11
	CASE CONVERT(FLOAT,ROUND(ISNULL((SELECT TOP 1 TRCODE  FROM {DATABASE}LG_{FIRMA}_{DONEM}_CLFLINE WHERE CLIENTREF = CLCARD.LOGICALREF AND DATE_ BETWEEN @TARIH_1 AND @TARIH_2 AND SIGN = 1 AND CANCELLED =0 ORDER BY DATE_ DESC,LOGICALREF DESC ),0),2)) WHEN 20 THEN ''Havale'' when 1 then ''Nakit'' when 4 then ''AlacakDekontu'' 
				when 61 then ''Çek'' when 62 then ''Senet'' else ''Diğer'' end [SonTahsilatTuru],

-- 12
	(select DATEADD(DAY,AVERAGEAGE,DATE_) FROM {DATABASE}LG_{FIRMA}_{DONEM}_CSROLL WHERE LOGICALREF IN (ISNULL((SELECT TOP 1 SOURCEFREF  FROM {DATABASE}LG_{FIRMA}_{DONEM}_CLFLINE WHERE CLIENTREF = CLCARD.LOGICALREF AND DATE_ BETWEEN @TARIH_1 AND @TARIH_2 AND SIGN = 1 AND CANCELLED =0 
	ORDER BY DATE_ DESC,LOGICALREF DESC ),0))) [SonTahsilatCekOrtVade],

	CONVERT(FLOAT,ROUND(ISNULL((SELECT TOP 1 AMOUNT  FROM ..{DATABASE}LG_{FIRMA}_{DONEM}_CLFLINE WHERE CLIENTREF = CLCARD.LOGICALREF AND DATE_ BETWEEN @TARIH_1 AND @TARIH_2 AND SIGN = 1 AND CANCELLED =0 ORDER BY DATE_ DESC,LOGICALREF DESC ),0),2)) [SonTahsilatTutari],
...
( SELECT USERNAME FROM L_CAPIUSER DT_USERS WHERE LOGICALREF IN (SELECT TOP 1 CARI.GORUSEN FROM {DATABASE}EMY_CARI_GORUSMELER CARI    WHERE CLCARD.LOGICALREF=CARI.CARI_REF ORDER BY CARI.TARIH DESC)) [SonGorusenKisi],
( SELECT TOP 1 CARI.GORUSULEN FROM {DATABASE}EMY_CARI_GORUSMELER CARI    WHERE CLCARD.LOGICALREF=CARI.CARI_REF ORDER BY CARI.TARIH DESC) [SonGorusulenKisi],
	( SELECT TOP 1 TARIH FROM {DATABASE}EMY_CARI_GORUSMELER CARI    WHERE CLCARD.LOGICALREF=CARI.CARI_REF ORDER BY CARI.TARIH DESC) [SonGorusmeTarihi],
	( SELECT TOP 1 ACIKLAMA FROM {DATABASE}EMY_CARI_GORUSMELER CARI    WHERE CLCARD.LOGICALREF=CARI.CARI_REF ORDER BY CARI.TARIH DESC) [SonGorusmeDetayi]

	FROM  {DATABASE}LG_{FIRMA}_CLCARD CLCARD
		LEFT JOIN  {DATABASE}LG_{FIRMA}_PAYPLANS PAYPLANS ON CLCARD.PAYMENTREF = PAYPLANS.LOGICALREF 
	WHERE CLCARD.ACTIVE =0  AND CLCARD.CODE LIKE ''120%'' AND  (@_CARIKOD='''' OR CLCARD.CODE LIKE REPLACE(@_CARIKOD,''*'',''%''))  
) KK
WHERE ROUND(KK.[ToplamBakiye],2)  >@_BAKIYE '
     ...
     EXECUTE sp_executesql @QUERY, @PARAMETRELER, @_BASTAR = @_BASTAR_VALUE, @_BITTAR = @_BITTAR_VALUE, @_CARIKOD = @_CARIKOD_VALUE, @_BAKIYE = @_BAKIYE_VALUE;
END
