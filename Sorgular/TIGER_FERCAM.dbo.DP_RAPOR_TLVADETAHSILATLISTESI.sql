...
GO
CREATE PROCEDURE DP_RAPOR_TLVADETAHSILATLISTESI @DB NVARCHAR(50), @FIRM NVARCHAR(3), @DONEM NVARCHAR(2) ,
...
BEGIN
     ...
     SET @PARAMETRELER = N'@_RAPTAR DATETIME,@_CARKOD NVARCHAR(50),@_BAKIYE FLOAT';
...
-- 5
	CONVERT(DATE,(SELECT DATEADD(DAY,ROUND((SUM(DATEDIFF(DAY,GETDATE(),B_VADE_TARIHI) * B_TUTAR ) / SUM(B_TUTAR)),0),GETDATE()) FROM {DATABASE}..DY_BorcTakip  WHERE B_TUTAR > 0 AND CLIENTREF = CLCARD.LOGICALREF  AND B_GIRIS_TARIHI BETWEEN @TARIH_1 AND @TARIH_2 AND K_ISLEM_TUTAR =0)) [Ortalama Vade],
-- 6	
	CONVERT(FLOAT,ROUND(ISNULL((SELECT SUM(B_TUTAR) FROM {DATABASE}..DY_BorcTakip WHERE CLIENTREF = CLCARD.LOGICALREF  AND B_GIRIS_TARIHI BETWEEN @TARIH_1 AND @TARIH_2 AND B_VADE_TARIHI BETWEEN @TARIH_1 AND @TARIH_2 AND K_ISLEM_TUTAR =0),0),2)) [Dolan Bakiye],
-- 7
	CONVERT(DATE,(SELECT DATEADD(DAY,ROUND((SUM(DATEDIFF(DAY,GETDATE(),B_VADE_TARIHI) * B_TUTAR ) / SUM(B_TUTAR)),0),GETDATE()) FROM {DATABASE}..DY_BorcTakip  WHERE B_TUTAR > 0 AND CLIENTREF = CLCARD.LOGICALREF  AND B_GIRIS_TARIHI BETWEEN @TARIH_1 AND @TARIH_2 AND B_VADE_TARIHI between @TARIH_1 AND @TARIH_2 AND K_ISLEM_TUTAR =0)) [Dolan Vade],
-- 8 	
	CONVERT(FLOAT,ROUND(ISNULL((SELECT TOP 1 B_TUTAR FROM {DATABASE}..DY_BorcTakip WHERE B_TUTAR > 0 AND CLIENTREF = CLCARD.LOGICALREF  AND B_GIRIS_TARIHI BETWEEN @TARIH_1 AND @TARIH_2 AND B_VADE_TARIHI > @TARIH_2 AND K_ISLEM_TUTAR =0 ORDER BY B_VADE_TARIHI),0),2)) [Dolacak Ilk Fatura Tutarı],
-- 9 	
	CONVERT(DATE, (SELECT TOP 1 B_VADE_TARIHI FROM {DATABASE}..DY_BorcTakip WHERE B_TUTAR > 0 AND CLIENTREF = CLCARD.LOGICALREF  AND B_GIRIS_TARIHI BETWEEN @TARIH_1 AND @TARIH_2 AND  B_VADE_TARIHI > @TARIH_2 AND K_ISLEM_TUTAR =0 ORDER BY B_VADE_TARIHI)) [Dolacak Ilk Fatura Vade],
-- 10	
	CONVERT(DATE,(SELECT TOP 1 DATE_ FROM {DATABASE}..LG_{FIRMA}_{DONEM}_CLFLINE WHERE CLIENTREF = CLCARD.LOGICALREF AND DATE_ BETWEEN @TARIH_1 AND @TARIH_2 AND SIGN = 1 AND CANCELLED =0 ORDER BY DATE_ DESC,LOGICALREF DESC )) [Son Tahsilat Tarihi],
-- 11
	CONVERT(FLOAT,ROUND(ISNULL((SELECT TOP 1 AMOUNT  FROM {DATABASE}..LG_{FIRMA}_{DONEM}_CLFLINE WHERE CLIENTREF = CLCARD.LOGICALREF AND DATE_ BETWEEN @TARIH_1 AND @TARIH_2 AND SIGN = 1 AND CANCELLED =0 ORDER BY DATE_ DESC,LOGICALREF DESC ),0),2)) [Son Tahsilat Tutarı],
	CONVERT(FLOAT,ROUND(ISNULL((SELECT SUM(B_TUTAR)  FROM {DATABASE}..DY_BorcTakip WHERE CLIENTREF = CLCARD.LOGICALREF  AND B_GIRIS_TARIHI BETWEEN @TARIH_1 AND @TARIH_2 AND B_VADE_TARIHI > @TARIH_2 AND K_ISLEM_TUTAR =0),0),2)) [Devreden Bakiye],
	CONVERT(DATE,(SELECT DATEADD(DAY,ROUND((SUM(DATEDIFF(DAY,GETDATE(),B_VADE_TARIHI) * B_TUTAR ) / SUM(B_TUTAR)),0),GETDATE()) FROM {DATABASE}..DY_BorcTakip  WHERE B_TUTAR > 0 AND CLIENTREF = CLCARD.LOGICALREF  AND B_GIRIS_TARIHI BETWEEN @TARIH_1 AND @TARIH_2 AND B_VADE_TARIHI > @TARIH_2 AND K_ISLEM_TUTAR =0)) [Devreden Vade]
...
', 8001, 4000)
     ...
     EXECUTE sp_executesql @QUERY, @PARAMETRELER, @_RAPTAR = @_RAPTAR_VALUE, @_CARKOD = @_CARKOD_VALUE, @_BAKIYE = @_BAKIYE_VALUE;
END
GO
