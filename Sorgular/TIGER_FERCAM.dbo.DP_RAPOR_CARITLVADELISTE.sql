...
GO
CREATE PROCEDURE DP_RAPOR_CARITLVADELISTE @DB NVARCHAR(50), @FIRM NVARCHAR(3), @DONEM NVARCHAR(2) ,
...
(
	...
	CONVERT(FLOAT,ROUND(ISNULL((SELECT SUM(CASE WHEN SIGN = 1 THEN AMOUNT ELSE 0-AMOUNT END) BAKIYE  FROM LG_{FIRMA}_{DONEM}_CLFLINE CLFLINE
			LEFT JOIN LG_{FIRMA}_{DONEM}_INVOICE INVOICE ON CLFLINE.SOURCEFREF = INVOICE.LOGICALREF AND CLFLINE.MODULENR = 4
			WHERE   CLFLINE.CANCELLED = 0 AND CLFLINE.CLIENTREF = CLCARD.LOGICALREF  AND ISNULL(INVOICE.FROMKASA,0) = 0 AND CLFLINE.DATE_ BETWEEN @TARIH_1 AND @TARIH_2),0),2)) [Toplam Bakiye],

	CONVERT(FLOAT,ROUND(ISNULL((SELECT SUM(B_TUTAR) FROM DY_BorcTakip WHERE CLIENTREF = CLCARD.LOGICALREF  AND B_GIRIS_TARIHI BETWEEN @TARIH_1 AND @TARIH_2 AND B_VADE_TARIHI BETWEEN @TARIH_1 AND @TARIH_2 AND K_ISLEM_TUTAR =0),0),2)) [Dolan Bakiye],

	 CONVERT(DATE,(SELECT DATEADD(DAY,ROUND((SUM(DATEDIFF(DAY,GETDATE(),B_VADE_TARIHI) * B_TUTAR ) / SUM(B_TUTAR)),0),GETDATE()) FROM DY_BorcTakip  WHERE B_TUTAR > 0 AND CLIENTREF = CLCARD.LOGICALREF  AND B_GIRIS_TARIHI BETWEEN @TARIH_1 AND @TARIH_2 AND B_VADE_TARIHI between @TARIH_1 AND @TARIH_2 AND K_ISLEM_TUTAR =0)) [Dolan Vade],
	CONVERT(FLOAT,ROUND(ISNULL((SELECT SUM(B_TUTAR) FROM DY_BorcTakip WHERE CLIENTREF = CLCARD.LOGICALREF  AND B_GIRIS_TARIHI BETWEEN @TARIH_1 AND @TARIH_2 AND B_VADE_TARIHI > @TARIH_2 AND K_ISLEM_TUTAR =0),0),2)) [Devreden Bakiye],
	CONVERT(DATE,(SELECT DATEADD(DAY,ROUND((SUM(DATEDIFF(DAY,GETDATE(),B_VADE_TARIHI) * B_TUTAR ) / SUM(B_TUTAR)),0),GETDATE()) FROM DY_BorcTakip  WHERE B_TUTAR > 0 AND CLIENTREF = CLCARD.LOGICALREF  AND B_GIRIS_TARIHI BETWEEN @TARIH_1 AND @TARIH_2 AND B_VADE_TARIHI > @TARIH_2 AND K_ISLEM_TUTAR =0)) [Devreden Vade],
	ISNULL(PAYPLANS.CODE,'''') [Tanımlı Vade Kodu],
	ISNULL(PAYPLANS.DEFINITION_,'''')  [Tanımlı Vade],
	CONVERT(DATE,(SELECT TOP 1 DATE_ FROM LG_{FIRMA}_{DONEM}_CLFLINE WHERE CLIENTREF = CLCARD.LOGICALREF AND DATE_ BETWEEN @TARIH_1 AND @TARIH_2 AND SIGN = 0 AND CANCELLED = 0 ORDER BY DATE_ DESC,LOGICALREF DESC )) [Son Ödeme Tarihi],

	CONVERT(FLOAT,ROUND(ISNULL((SELECT TOP 1 AMOUNT  FROM LG_{FIRMA}_{DONEM}_CLFLINE WHERE CLIENTREF = CLCARD.LOGICALREF AND DATE_ BETWEEN @TARIH_1 AND @TARIH_2 AND SIGN = 0 AND CANCELLED = 0 ORDER BY DATE_ DESC,LOGICALREF DESC ),0),2)) [Son Ödeme Tutarı],
	'''' [Nakit Ödeme],
	'''' [Çek Ödeme]
	FROM LG_{FIRMA}_CLCARD CLCARD
		LEFT JOIN LG_{FIRMA}_PAYPLANS PAYPLANS ON CLCARD.PAYMENTREF = PAYPLANS.LOGICALREF 

	WHERE CLCARD.ACTIVE =0 AND (@_CARKOD='''' OR CLCARD.CODE LIKE REPLACE(@_CARKOD,''*'',''%'')) -- AND CLCARD.LOGICALREF = 90
) KK
...
', 4001, 4000)
     SET @QUERY3 = SUBSTRING('--- 320 ALACAK VADE ANALİZİ
DECLARE @TARIH_1 DATE
DECLARE @TARIH_2 DATE
DECLARE @CLIENTREF integer
DECLARE @B_PAYPLANREF integer
