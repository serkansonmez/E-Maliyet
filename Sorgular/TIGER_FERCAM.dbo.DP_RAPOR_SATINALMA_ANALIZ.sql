...
GO
CREATE PROCEDURE DP_RAPOR_SATINALMA_ANALIZ @DB NVARCHAR(50), @FIRM NVARCHAR(3), @DONEM NVARCHAR(2) ,
                                      @_BASTAR_VALUE DATETIME,
                                      @_BITTAR_VALUE DATETIME,
                                      @_CARIKODU_VALUE NVARCHAR(50),
                                      @_MLZKODU_VALUE NVARCHAR(50)
...
BEGIN
     ...
     SET @PARAMETRELER = N'@_BASTAR DATETIME,@_BITTAR DATETIME,@_CARIKODU NVARCHAR(50),@_MLZKODU NVARCHAR(50)';
     ...
     SET @QUERY1 = SUBSTRING('(SELECT     CASE WHEN ITEMS.ACTIVE = 0 THEN ''Kullanımda'' ELSE ''Kullanım Dışı'' END AS [Cari Statusü], 
					  ...
					  WHEN 7 THEN ''07.TEMMUZ'' WHEN 8 THEN ''08.AĞUSTOS'' WHEN 9 THEN ''09.EYLÜL'' WHEN 10 THEN ''10.EKİM'' WHEN 11 THEN ''11.KASIM'' ELSE ''12.ARALIK'' END AS  [Fiş Ayı],	  CASE WHEN STLINE.TRCODE IN(6) THEN (STLINE.AMOUNT*STLINE.UINFO2/STLINE.UINFO1)*-1 ELSE (STLINE.AMOUNT*STLINE.UINFO2/STLINE.UINFO1) END   AS Miktar, 
                      STLINE.VAT AS [Kdv], CASE WHEN STLINE.TRCODE IN(6) THEN (STLINE.VATMATRAH + STLINE.DIFFPRICE)*-1 ELSE STLINE.VATMATRAH + STLINE.DIFFPRICE END  AS [Kdv Hariç],CASE WHEN STLINE.TRCODE IN(6) THEN STLINE.VATAMNT*-1 ELSE STLINE.VATAMNT END  AS [Kdv Tutarı], 
                      ''Malzeme'' AS [Satır Tipi],ISNULL((SELECT CODE FROM {DATABASE}LG_SLSMAN WHERE LOGICALREF=STLINE.SALESMANREF),'''') AS [Satıcı Kodu],ISNULL((SELECT DEFINITION_  FROM {DATABASE}LG_SLSMAN WHERE LOGICALREF=STLINE.SALESMANREF),'''') AS [Satıcı Adı]
						,CASE WHEN STLINE.TRCODE IN(6) THEN (STLINE.OUTCOST * (STLINE.AMOUNT*STLINE.UINFO2/STLINE.UINFO1)) * -1 ELSE (STLINE.OUTCOST * (STLINE.AMOUNT*STLINE.UINFO2/STLINE.UINFO1)) END AS Maliyet,
						STLINE.DIFFPRICE*-1 AS [Satış Fiyat Farkı],
						CASE WHEN STLINE.TRCODE IN(6) 
...
FROM         {DATABASE}LG_{FIRMA}_{DONEM}_STLINE STLINE WITH(NOLOCK) LEFT OUTER JOIN
            ...
            WHERE     (STLINE.LINETYPE = 0) AND (STLINE.CANCELLED = 0) and STLINE.TRCODE IN (1,6)
AND (@_CARIKODU='''' OR CLCARD.CODE LIKE REPLACE(@_CARIKODU,''*'',''%'')) AND (@_MLZKODU='''' OR ITEMS.CODE LIKE REPLACE(@_MLZKODU,''*'',''%'')) 
...
AND (@_CARIKODU='''' OR CLCARD.CODE LIKE REPLACE(@_CARIKODU,''*'',''%'')) AND (@_MLZKODU='''' OR SRVCARD.CODE LIKE REPLACE(@_MLZKODU,''*'',''%'')) 
...
AND (STLINE.DATE_ BETWEEN @_BASTAR AND @_BITTAR))', 4001, 4000)
     ...
     EXECUTE sp_executesql @QUERY, @PARAMETRELER, @_BASTAR = @_BASTAR_VALUE, @_BITTAR = @_BITTAR_VALUE, @_CARIKODU = @_CARIKODU_VALUE, @_MLZKODU = @_MLZKODU_VALUE;
END
GO
