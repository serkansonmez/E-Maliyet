...
namespace E_Maliyet.Membership.Pages
{
    using Serenity;
    ...
    using System.Linq;
    using System.Text;
    using E_Maliyet.Administration.Repositories;
    using E_Maliyet.Common;
    ...
    public partial class AccountController : Controller
    {
        ...
        public Result<ServiceResponse> Login(LoginRequest request)
        {
            ...
            return this.ExecuteMethod(() =>
            {
                ...
                if (Dependency.Resolve<IAuthenticationService>().Validate(ref username, request.Password))
                {
                  ...
                  //  WebSecurityHelper.SetAuthenticationTicket(username, true);
        ...
        private void CheckTwoFactorAuthentication(string username, LoginRequest request)
        {
            ...
            // Firma Bilgileri Bu alana girildi
            user.TigerFirma = request.TigerFirma.PadLeft(3, '0');
            user.TigerDonem = request.TigerDonem.PadLeft(2, '0');
            // 

            List<TigerFirmalarRow> odemeBilgileri = new List<TigerFirmalarRow>();
            using (var connection = SqlConnections.NewFor<TigerFirmalarRow>())
            {
                var m = TigerFirmalarRow.Fields;
                odemeBilgileri = connection.List<TigerFirmalarRow>(q => q
                  .Select(m.Name)
                 .Where("[Nr] ='" + request.TigerFirma + "' ") );
            }
            user.TigerFirmaAdi = odemeBilgileri[0].Name;

            List<TigerDonemlerRow> DonemBilgileri = new List<TigerDonemlerRow>();
            using (var connection = SqlConnections.NewFor<TigerDonemlerRow>())
            {
                var m = TigerDonemlerRow.Fields;
                DonemBilgileri = connection.List<TigerDonemlerRow>(q => q
                  .Select(m.Begdate)
                 .Where("[FIRMNR] ='" + request.TigerFirma + "' and  [Nr] = '" + request.TigerDonem + "'"));
            }
            user.DonemBaslangicTarihi = Convert.ToDateTime(DonemBilgileri[0].Begdate);
                 ...
                 (user.TwoFactorAuth == Administration.TwoFactorAuthType.Email)))
            {
                ...
                if (user.TwoFactorAuth == Administration.TwoFactorAuthType.SMS)
                {
                    ...
                    Dependency.Resolve<Administration.ISMSService>().Send(
                        phoneNumber: mobile,
                        text: "Please use code " + data.TwoFactorCode + " for E_Maliyet login.",
                        reason: "Sent by E_Maliyet system for two factor authenication by SMS (" + user.Username + ")");
                ...
                else
                {
                    EmailHelper.Send(
                        address: user.Email,
                        subject: "Your two-factor authentication code for E_Maliyet login",
                        body: "Please use code " + data.TwoFactorCode + " for E_Maliyet login.");
        ...
        public ActionResult ImpersonateAs(string token)
        {
            ...
            using (var br = new BinaryReader(ms))
            {
                ...
                ((UserDefinition)Authorization.UserDefinition).TigerFirma = "1234";
